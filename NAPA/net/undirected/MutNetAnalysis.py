...
class MutGraph(object):
    def __init__(self,g=None,epiNetFile=None,fileIsNet=None,fromGraph=None):
        if fromGraph == True and epiNetFile==None:
            ...
            stderr_write([ "Parsing into graph format!"])
            if fileIsNet:
            ...
            stderr_write(["FromGraphInit?", fromGraph, 
                          "isFile?", os.path.isfile(epiNetFile)])
...
def main():
    ...
    args = parseArgs()
    mg = MutGraph(epiNetFile=args.epiPairFileName,fileIsNet=True,
