projDir=/home/vbeleva/phylogeny/pairs
runDir=$projDir/data/old
workDir=$projDir/directedPairs
globalFasta=$runDir/PSE4-TEM-SHV_noLen.cds.prot.fasta
maxTrees=$2
maxRuns=$1
echo $maxRuns $maxTrees
outPath=$workDir/results/$maxRuns
mkdir -p $outPath
phenoFile=$runDir/TEM_pheno.txt


distThreshs=( 0.05 0.1 )
for d in "${distThreshs[@]}"
do
    nohup python get_directedPairs_single.py -d $runDir -r $maxRuns -m $maxTrees \
	-g $globalFasta -p $phenoFile -o $outPath -t $d \
	>& getDirPairs.$maxRuns.$maxTrees.$d.oe &
done
