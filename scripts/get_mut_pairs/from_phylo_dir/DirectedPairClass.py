...
class PhyloEdge(object):
    ...
    def build_EdgeTree(self):
        ...
        stderr_write(["\tBuilding edge followers/precursors", datetime.now()])
    ...
    def check_pheno(self):
        ...
        return phenoCheck and self.check_pos()
...
class DirectedPairSet(object):
# members = ['phyloTree', 'ancNode', 'ancEdge', 'edgePairs'
    def __init__(self, nwkFile, fastaFile, posList, pos2wt,tip2pheno, phenoClades,
        ...
        #stderr_write(phenoClades)
        self.phenoLeafNodes = filter(lambda phyloNode: \
                                     phyloNode.name in flatten(phenoClades.values()),
        ...
        stderr_write(['\tMRCA 2be',self.ancNode.name, 
                          '\t'+str(datetime.now())])

        intPhenoFile =  nwkFile.replace("int_nwk.tree","internalStates.txt")
        intPhenoFile =  intPhenoFile.replace("int_subPheno.tree","internalStates.txt")
        with open(intPhenoFile,'rb') as pf:
            stderr_write(["Internal node pheno",intPhenoFile])
        ...
        stderr_write(["nodes and internal nodes with phenotype",len(self.node2pheno)])
    ...
    def get_phenoMuts(self):
        ...
        stderr_write(["phenotype mutations"] + list(self.phenoMutations))

    def get_dirPairs(self,distThresh):
        mutPair2phyloEdgePairs = defaultdict(list) 
        mut2phyloEdgePairs = defaultdict(list)
        allEdges = [self.ancEdge]+self.ancEdge.followers
        for edge in allEdges:
            edge.assign_ordered_precursors()
        stderr_write(["\t\tNumber of tree edges:", len(allEdges)])

        for startEdge in self.ancEdge.followers:
            if startEdge.isLeafEdge(): continue
            if not startEdge.check_pheno(): continue
            if startEdge.check_dist(startEdge, distThresh):
                ...
                if len(startEdgeSortedMutations)>1:
                    ...
                    startEdgeSortedMutations.reverse()
                    startEdgeMutationPairs += listPairs(startEdgeSortedMutations)
                    for mutPair in startEdgeMutationPairs:
                        if getInt(mutPair[0]) == getInt(mutPair[1]): continue
            ...
            for followEdge in startEdge.followers:
                ...
                if (not followEdge.check_pheno()):
                    continue
                if not startEdge.check_dist(followEdge,distThresh):
                    continue
        ...
        stderr_write(["\t\tNumber of AA mutations for pheno:",len(allMutations)])

        #stderr_write(list(set(flatten(mutPair2phyloEdgePairs.keys()))))
        stderr_write(["\t\tNumber of AA mutations for phenotype:",
                      len(allMutations)])
        
        allAAmutEdges = set([edge for edge in allEdges if edge.check_pheno()])
        ...
        for mutPair in mutPair2phyloEdgePairs:
            ...
            mutPair = MutationPair(phyloEdgePairs = list(set(\
                                        mutPair2phyloEdgePairs[mutPair])),
                                        mutationPair = (self.mut2obj[mutPair[0]],
                                                        self.mut2obj[mutPair[1]]),
                                        allEdges=allAAmutEdges)
            mutPair.assign_lineageDistribution()
            self.dirPairs.append(mutPair)

        stderr_write(["\t\tPairwise lineage dsitributions assigned"])
        stderr_write(["\t\tBuild contingencies."])
        for mutPair in self.dirPairs:
            mutPair.get_contingencyTable()
                
    def __repr__(self):
        outStr='\t'.join(['_Mut1_','_Mut2_','edgePairNum',
                          'contTable(common_1_2_other)',
                          "FisherPvalLess","FisherPvalMore"])+'\n'
        
        for dirPairIdx in range(len(self.dirPairs)):
            dirPair = self.dirPairs[dirPairIdx]

            if len(dirPair.contingencyTable)<2:
                continue
            
            if len(flatten(dirPair.contingencyTable))<4:
                continue
            if dirPair.contingencyTable[0][1]==0:
                if dirPair.contingencyTable[1][0]==0:
                    if dirPair.contingencyTable[0][0]<=2:
                        continue

            toPrint = [str(dirPair.mutationPair[0]), str(dirPair.mutationPair[1])]
            toPrint.append(str(len(dirPair.phyloEdgePairs)))
            toPrint.append(str(dirPair.contingencyTable))
            toPrint.append(str(dirPair.pValLess))
            toPrint.append(str(dirPair.pValMore))
            outStr+= '\t'.join(toPrint)+'\n'
